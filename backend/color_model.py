...
TRAINED_COLOR_WEIGHTS = np.array([2.46472664e-01, 2.69276370e-01, 1.42605799e-01, 1.29811004e-01, 9.77995229e-05, 2.11736362e-01])

def init_genetic_weights():
    return np.array([1/6] * 6)
    #return np.array([0.15758107, 0.22912373, 0.0099046, 0.24909835, 0.16754662, 0.18674563])

    #return np.array([0.040433,   0.44685666, 0.2766772,  0.00641356, 0.10588637, 0.12373321])
...
def run_single_game(weights, game_id="sim_genetic", verbose=False, scenario=None, trace=False):
    ...
    if scenario is None:
        ...
        session.score_types, session.score_types_names, session.score_types_colors = gameStart.select_scoring_cards()
    else:
        ...
        session.score_types_colors = scenario["score_types_colors"]

    session.current_card = session.deck[0] if session.deck else None

    ordered_weights = copy.deepcopy(weights)
    for i in range(len(session.score_types_colors)):
        color = session.score_types_colors[i]
        if color == "Green":
            ordered_weights[i] = weights[0]
        elif color == "Blue":
            ordered_weights[i] = weights[1]
        elif color == "Red":
            ordered_weights[i] = weights[2]
        elif color == "Misc":
            ordered_weights[i] = weights[3]
    

    if verbose:
        print(f"=== Starting Game {game_id} ===")
        print("Scoring cards:", session.score_types_names)
        print("Card colors:", session.score_types_colors)
    ...
    for season in range(4):
        ...
        while player.season_time > 0 and player.deck_index < len(session.deck):
            ...
            if new_grid is None and player.ruins_required:
                ...
                new_grid = best_riftlands_tile_placement(player.current_grid, season, ordered_weights, reward_fns)
...
if __name__ == "__main__":
    ...
    baseline_results = run_many_games(w0, n=100, verbose=False)
